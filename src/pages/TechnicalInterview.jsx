...
export default function TechnicalInterview() {
  ...
  const handleStartMock = async (e) => {
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error(data.message || 'Unable to instantiate technical mock session.');
      }
      ...
      if (data.questions && data.questions.length > 0) {
        ...
        throw new Error("No questions retrieved. Please verify prompt settings.");
      }
    } catch (err) {
  ...
  const handleEvaluateAnswer = async () => {
    if (!userAnswer.trim()) {
      alert("Please provide some response elements to submit for evaluation.");
      return;
    }
    ...
    try {
      await fetch('/api/ai/interviews/save', {
        ...
        body: JSON.stringify({
          ...
          details: roundProgress.map((p, index) => ({
            question: p.question,
            answer: p.answer,
  ...
  return (
    <div className="bg-[#0b0f19] min-h-screen text-slate-100 py-8" id="page-technical">
      <div className="max-w-4xl mx-auto px-6">
        ...
        <div className="mb-6 flex justify-between items-center bg-slate-900 border border-slate-800 rounded-xl px-5 py-3">
          <div className="flex items-center gap-2 text-xs text-slate-400 font-sans">
            ...
            <span className="font-semibold text-white">Technical Interview Room</span>
          </div>
          <div className="flex items-center gap-2 text-white text-xs font-semibold">
            <Cpu className="w-4 h-4 text-[#f97316] animate-pulse" />
            <span className="font-mono font-bold">Simulator Active</span>
          </div>
        </div>
        ...
        {questions.length === 0 && !isCompleted && (
          ...
          >
            <div className="flex items-start gap-4 border-b border-slate-800 pb-5">
              ...
              <div>
                <h1 className="text-base font-extrabold text-white tracking-wide uppercase font-mono">// TECHNICAL EVALUATION ROUND</h1>
                <p className="text-xs text-slate-400 font-sans mt-0.5 leading-relaxed font-normal">
                  Generate core programming and architecture questions custom-tailored to your stack.
                </p>
              </div>
            </div>
            ...
            <form onSubmit={handleStartMock} className="space-y-4">
              ...
              <div>
                ...
                <input
                  ...
                  placeholder="e.g. Redux Toolkit, SQL schemas, GraphQL, Docker, Microservices"
                  className="w-full px-3.5 py-2.5 bg-slate-950 border border-slate-850 focus:border-[#f97316] rounded-xl text-xs outline-none text-white transition-all placeholder:text-slate-650"
                  id="tech-skills-focus"
                />
              </div>
                ...
                >
                  {loadingQuestions ? (
                    <>
                      ...
                      <span>Initializing Simulator Dialogue...</span>
                    </>
                  ) : (
                    <>
                      <span>Start Custom Mock Round</span>
            ...
            >
              <div className="space-y-2">
                <span className="inline-flex items-center gap-1.5 px-2.5 py-0.5 bg-[#f97316]/10 border border-[#f97316]/20 text-[#f97316] rounded-full text-[9px] font-mono font-bold uppercase tracking-wider">
                  ...
                  ACTIVE CONVERSATIONAL QUESTIONS
                </span>
                <h2 className="text-sm md:text-base font-bold text-white leading-relaxed">
                  {questions[currentIdx].text}
                </h2>
                <p className="text-[11px] text-slate-500 font-sans italic font-normal">
                  <strong>Assessment Target:</strong> {questions[currentIdx].rationale}
                </p>
              </div>

              {/* Rationale key criteria */}
              <div className="bg-slate-950/80 border border-slate-850 rounded-xl p-4 font-sans text-xs">
                <span className="font-bold text-slate-455 text-slate-400 block mb-2">// Checked concepts reviewed:</span>
              ...
              <div className="space-y-2">
                ...
                <textarea
                  ...
                  placeholder="Draft your clear database diagram specifications, architectural design layers, complexity evaluations, or custom snippets here..."
                  ...
                  id={`tech-textarea-answer-${currentIdx}`}
                />
              ...
              >
                <div className="flex items-center justify-between border-b border-slate-850 pb-3">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="font-sans font-extrabold text-[#f97316] text-[10px] uppercase tracking-wide">Conversational Diagnostic Scorecard</span>
                  </div>
                  <div className="bg-[#f97316]/10 border border-[#f97316]/20 px-3.5 py-1 rounded-full text-xs font-mono font-bold text-[#f97316]">
                    ACCURACY SCORE: {activeFeedback.score} / 100
                  </div>
                </div>

                <div className="space-y-3 font-sans text-xs">
                  <div>
                    <h4 className="font-bold text-emerald-400 uppercase tracking-wide text-[10px] font-mono">✓ Strengths / Covered Terms</h4>
                    <p className="text-slate-350 mt-1 leading-relaxed font-sans">{activeFeedback.strengths}</p>
                  </div>

                  <div className="border-t border-slate-850 pt-3">
                    <h4 className="font-bold text-rose-450 text-rose-400 uppercase tracking-wide text-[10px] font-mono">⚡ Structural Pitfalls / Gaps</h4>
                    <p className="text-slate-350 mt-1 leading-relaxed font-sans">{activeFeedback.weaknesses}</p>
                  </div>
        ...
        {isCompleted && (
          ...
          >
            ...
            <div>
              <h1 className="text-lg font-bold text-white uppercase tracking-wider font-mono">TECHNICAL INTERVIEW COMPLETED</h1>
